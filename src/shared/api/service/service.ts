import type { TokensResultDto } from "@jobio-org/api-client";
...
class ApiWithRefreshManager implements Api {
    ...
    private handleExpiredToken = async (response: ApiError): Promise<Response> => {
        try {
            ...
            if (!session?.accessToken) {
                return await this.createErrorResponsePromise(response);
            }
            ...
            if (repeatedRequestResponse.status === UNAUTHORIZED_ERROR) {
                this.authenticationErrorCallback?.();
                throw new Error("Repeated request unauthorized");
            }
    ...
    private createErrorResponsePromise = (response: ApiError) =>
        new Promise<Response>((_resolve, reject) => reject(response));
